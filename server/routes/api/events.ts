...
router.post<{
    ...
    async (req, res) => {
        const [matchScouting, answers, questions, pictures, teams, matches] = await Promise.all([
        ...
        const matchesLeft = matchesData.map(m => {
            const find = (t: number): number | null => {
                ...
                return matchScoutingData.find(s => s.team === t && s.matchNumber === m.match_number && s.compLevel === m.comp_level) ? null : t;
            }
            ...
            return {
                ...
                teams: [
                    ...m.alliances.blue.team_keys.map(t => parseInt(t.substring(3))).map(find).filter(Boolean),
                    ...m.alliances.red.team_keys.map(t => parseInt(t.substring(3))).map(find).filter(Boolean),
                ]
            }
        }) as {
            match: number;
        ...
        const questionsLeft = teamsData.map(t => {
            return {
                team: t.team_number,
                questions: questionsData.filter(q => !answersData.find(a => a.teamNumber === t.team_number && a.questionId === q.id)).map(q => q.key),
            }
        });
