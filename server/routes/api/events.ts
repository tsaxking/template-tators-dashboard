...
router.post<{
    ...
    if (matchScouting.isErr()) {
        ...
        return res.sendStatus('unknown:error', { error: matchScouting.error });
    }
    if (teamsResult.isErr()) {
        console.error(teamsResult.error);
        return res.sendStatus('unknown:error', { error: teamsResult.error });
    }
    if (!teamsResult.value) return res.sendStatus('tba:invalid-path');
    if (matchesResult.isErr()) {
        console.error(matchesResult.error);
        return res.sendStatus('unknown:error', { error: matchesResult.error });
    }
