    ...
    const climbData = (await Promise.all(events.map(async (e) => {
        ...
        const numClimbs = matches.map((m) => {
            ...
            switch (year) {
                case 2022:
                    return [
                        pullClimbs2022(m.score_breakdown.red as any),
                        pullClimbs2022(m.score_breakdown.blue as any),
                    ];
                case 2016:
                    return [
                        pullClimbs2016(m.score_breakdown.red as any),
                        pullClimbs2016(m.score_breakdown.blue as any),
                    ];
                case 2017:
                    return [
                        pullClimbs2017(m.score_breakdown.red as any),
                        pullClimbs2017(m.score_breakdown.blue as any),
                    ];
                case 2018:
                    return [
                        pullClimbs2018(m.score_breakdown.red as any),
                        pullClimbs2018(m.score_breakdown.blue as any),
                    ];
                default:
                    return null;
            }
