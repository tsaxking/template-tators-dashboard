...
export const runTests = async () => {
    Deno.test('Database Speed and Reliability', async () => {
        const num = 1000;
        const result = await runTask<number>('/scripts/tests/db-speed.ts', 'test', num.toString());
        log('Result:', result);
    ...
    Deno.test('Run async task functionality', async () => {
        const asyncTest = await runTask<string[]>('/scripts/tests/run-task-test.ts', 'asyncFn', 'a', 'b', 'c');
    ...
    Deno.test('Run sync task functionality', async () => {
        const syncTest = await runTask<string[]>('/scripts/tests/run-task-test.ts', 'syncFn', 'a', 'b', 'c');
